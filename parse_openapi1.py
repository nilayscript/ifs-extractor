...
def parse_openapi_spec(file_path: str) -> dict:
    ...
    # Extract all schemas (pass base_url for lookup API URL construction)
    base_url = result['api_info']['base_url']
    result['schemas'] = extract_all_schemas(data, base_url)
    result['summary']['total_schemas'] = len(result['schemas'])

    return result


def extract_all_schemas(spec: dict, base_url: str = '') -> dict:
    ...
    # First pass: collect all object schemas (to find lookup response fields later)
    # Lookup entity schemas are named like "Lookup_IsoCountry_EntityType"
    object_schemas = {}
    for schema_name, schema_def in schema_defs.items():
        if 'properties' in schema_def and '_EntityType' in schema_name and 'Update' not in schema_name and 'Insert' not in schema_name:
            props = []
            for prop_name, prop_def in schema_def.get('properties', {}).items():
                prop_info = {'key': prop_name, 'type': prop_def.get('type', 'unknown')}
                if 'maxLength' in prop_def:
                    prop_info['maxLength'] = prop_def['maxLength']
                if 'format' in prop_def:
                    prop_info['format'] = prop_def['format']
                props.append(prop_info)
            # Store by lookup name (e.g., "Lookup_IsoCountry_EntityType" -> "Lookup_IsoCountry")
            short_name = schema_name.replace('_EntityType', '')
            object_schemas[short_name] = props

    # Second pass: collect all lookup API endpoints from paths
    # Real lookups have endpoints like "/Lookup_IsoCountry_EntitySet"
    lookup_endpoints = set()
    for path in paths.keys():
        if 'Lookup_' in path and '_EntitySet' in path and '(' not in path:
            # Extract lookup name: "/Lookup_IsoCountry_EntitySet" -> "Lookup_IsoCountry"
            lookup_name = path.lstrip('/').replace('_EntitySet', '')
            lookup_endpoints.add(lookup_name)

    for schema_name, schema_def in schema_defs.items():
        # Skip enum types and simple types
        if schema_def.get('type') == 'string' and 'enum' in schema_def:
            # Extract lookup name from schema name
            lookup_name = schema_name.split('.')[-1]

            # A lookup needs API call ONLY if it has BOTH:
            # 1. An _EntityType schema (defines response fields)
            # 2. An _EntitySet API endpoint (the API actually exists)
            has_entity_type = lookup_name in object_schemas
            has_api_endpoint = lookup_name in lookup_endpoints
            is_real_lookup = has_entity_type and has_api_endpoint

            enum_schema = {
                'type': 'enum',
                'enumType': 'lookup' if is_real_lookup else 'fixed',
                'needsApiCall': is_real_lookup,
                'values': schema_def.get('enum', [])
            }

            # For real lookup enums, add API URL and response fields
            if is_real_lookup and base_url:
                enum_schema['lookupApiUrl'] = f"{base_url}/{lookup_name}_EntitySet"
                enum_schema['lookupResponseFields'] = object_schemas[lookup_name]

            schemas[schema_name] = enum_schema
...
def generate_simplified_output(parsed_data: dict) -> dict:
    ...
    def get_enum_details_from_ref(ref: str) -> dict:
        """Get enum details from a $ref if it points to an enum schema.

        Returns dict with enumType, needsApiCall, enumValues, and lookup details if applicable.
        Returns empty dict if not an enum.
        """
        if not ref:
            return {}

        # Extract schema name from $ref (e.g., "#/components/schemas/IfsApp.CustomerHandling.Lookup_IsoCountry")
        ...
        enum_details = {
            'enumType': schema.get('enumType', 'fixed'),
            'needsApiCall': schema.get('needsApiCall', False),
        }

        # For fixed enums, include the values directly
        if not enum_details['needsApiCall']:
            enum_details['enumValues'] = schema.get('values', [])
        else:
            # For lookup enums, include API URL and response fields
            enum_details['lookupApiUrl'] = schema.get('lookupApiUrl', '')
            if schema.get('lookupResponseFields'):
                enum_details['lookupResponseFields'] = schema.get('lookupResponseFields')
                ...
                if resp.get('properties'):
                    ...
                    for field_name, field_info in props.items():
                        ...
                        if isinstance(field_info, dict):
                            ...
                            # Check for enum/reference type and add enum details
                            if '$ref' in field_info:
                                field_entry['$ref'] = field_info['$ref']
                                enum_details = get_enum_details_from_ref(field_info['$ref'])
                                if enum_details:
                                    field_entry.update(enum_details)
    ...
    def process_api(api: dict, include_nested: bool = False, nested_list: list = None, entity_name_override: str = None) -> dict:
        ...
        # Add nested entities if provided (deep copy to avoid shared references)
        if include_nested and nested_list:
            result['nested_entities'] = copy.deepcopy(nested_list)
...
def main():
    ...
    # If specific file provided, parse only that file
    if len(sys.argv) > 1:
    ...
    if not files_to_parse:
        print("No JSON files found to parse.")
        sys.exit(1)

    print(f"Found {len(files_to_parse)} file(s) to parse\n")
    ...
    for file_path in files_to_parse:
        ...
        # Output file path: parsed_data/<lowercase_name>.json
        file_name = os.path.basename(file_path)
        output_name = os.path.splitext(file_name)[0].lower() + '.json'
        output_path = os.path.join(parsed_data_dir, output_name)

        # Write simplified output only
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(simplified_data, f, indent=2, ensure_ascii=False)
        print(f"Output written to: {output_path}")

        # Print summary
        print(f"\nAPI Title: {parsed_data['api_info']['title']}")
        print(f"Base URL: {parsed_data['api_info']['base_url']}")
        print(f"Total Endpoints: {parsed_data['summary']['total_endpoints']}")
        ...
        print(f"Methods: {parsed_data['summary']['methods_count']}")
        print()

    print("=" * 60)
    print(f"Parsing complete. {len(files_to_parse)} file(s) processed.")
    print("=" * 60)
